 ...
 * ("גנבנו גם אותן"), so the year line stays bare.
 */
...
export default function Footer({ brand, tagline, legalLinks }: FooterProps) {
  ...
  return (
    <footer className="border-t border-accent/15 bg-surface px-6 py-10">
      <div className="max-w-6xl mx-auto flex flex-col sm:flex-row items-center justify-between gap-6 text-center sm:text-right">
        <div className="space-y-1.5">
          <p className="gold-text font-display font-bold">{brand}</p>
          {tagline && <p className="text-sm text-muted">{tagline}</p>}
          <p className="text-xs text-muted">© {year} · נבנה בזהירות. הופעל בזדון.</p>
        </div>

        {legalLinks && legalLinks.length > 0 && (
          <nav className="flex flex-wrap items-center justify-center gap-x-5 gap-y-2 text-sm">
            {legalLinks.map((l) => (
              <a key={l.href} href={l.href} className="text-muted hover:text-accent transition-colors">
                {l.label}
              </a>
            ))}
          </nav>
